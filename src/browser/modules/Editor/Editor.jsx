...
export class Editor extends Component {
  ...
  execCurrent() {
    ...
    var cmd_search_fuzzy = 'CALL ga.es.queryNode(\'{"from":0, "size":10, "query":{"multi_match":{"query":"' + this.getEditorValue() + '", "fields" : ["name", "description", "subject_1", "subject_2"], "fuzziness": "AUTO"}}}\') YIELD node, score\n' +
      'CALL ga.es.queryRelationship(\'{"from":0, "size":10, "query":{"multi_match":{"query":"' + this.getEditorValue() + '", "fields" : ["name", "description", "subject_1", "subject_2"], "fuzziness": "AUTO"}}}\') YIELD relationship\n' +
      'RETURN collect(distinct node), collect(distinct relationship), score'
    ...
    // this.execCommand(cmd_search_fuzzy)
    // this.execCommand(cmd_search)
    this.execCommand(cmd_search_node)
    this.clearEditor()
    this.setState({
      notifications: [],
      historyIndex: -1,
